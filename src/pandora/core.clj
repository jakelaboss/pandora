...
"
  [letargs & body]
  (let [letargs# (let-binding-transform (concat '(this (meta-object. nil)) letargs))
...
(defn resolver [box]
  (cond (fn? box) (var-get box)
        (var? box) (var-get box)
        (symbol? box) (or (resolve box) box)
        :else (throw (ex-info "Invalid resolver" {:box box}))))
...
(let [this (meta-object. nil)]
  (.setVal this 401)
  (.getVal this))

(eval `(let [~@(let-binding-transform (concat '(this nil) '(y 1 i 2)))]
         ~'y#))
...
(defmacro p-eval
  ...
  "
  [box expr]
  `(binding [p-eval-tunnel '~box]
     (eval `(~'with-all-p ~p-eval-tunnel
                          ~~expr))))

(defn with-all-p* [box & body]
  `(p-eval ~box
           `(with-all-p ~'~box
              ~@'~body)))

;; so if we want this to work as a function, where
;; we can pass in the box and the expression, we can
